...
export function sanitizeStateCode(value: string) {
  return sanitizePlainText(value, { maxLength: 20 }).toUpperCase();
}

export function sanitizeFileName(fileName: string) {
  const sanitizedName = sanitizePlainText(fileName, { trim: false }).toLowerCase().replace(SAFE_FILENAME_CHARACTERS, "-");

  return sanitizedName.replace(/-+/g, "-").replace(/^-|-$/g, "") || "upload";
}

export function sanitizePersistedValue<T>(value: T): T {
  if (typeof value === "string") {
    return sanitizeMultilineText(value, { trim: false }) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => sanitizePersistedValue(item)) as T;
  }
  ...
  if (value && typeof value === "object") {
    return Object.fromEntries(
      Object.entries(value as Record<string, unknown>).map(([key, nestedValue]) => [
        key,
        sanitizePersistedValue(nestedValue),
      ]),
    ) as T;
  }
